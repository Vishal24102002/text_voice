...
def player(file_nam):
    pygame.init()
    pygame.mixer.init()
    pygame.mixer.music.load(file_nam)
...
def prin():
    ...
    if (len(txt)!=0 and len(text1.get())==0):
        speak(txt, n, s, v)
        print("loop")
    elif (len(txt)==0 and len(text1.get())==0):
...
voice_choice = ctk.CTkOptionMenu(Vikas1, values=["Voice 1", "Voice 2", "Voice 3", "Voice 4", "Voice 5"],
                                 variable=voice_type, width=50, command=voice,
                                 dropdown_hover_color=("orange", "yellow"))
voice_choice.grid(row=1, column=2)

voice_speed = ctk.StringVar()
voice_speed.set("Choose speed")
voice_cho = ctk.CTkOptionMenu(Vikas1, values=["VERY-FAST", "FAST", "NORMAL", "SLOW", "VERY-SLOW"], variable=voice_speed,
                              width=50, dropdown_hover_color=("orange", "yellow"), command=speed)
voice_cho.grid(row=2, column=2)

doc = ctk.CTkOptionMenu(Vikas1, values=["PDF", "TEXT","Music"], command=document, width=50)
